...
%   Output: Apix is the pixelated 'photons history' array.
    ...
    for i = 1:size
        ...
        for j = 1:sum(A(:,2,i)>0)
            % Evaluate the index (2 is the voxel size in mm)
        ...
        % Find the voxels indexes with values > 0
...
%         ii = mod(nn, 100);
        ...
        if j >= 1
            ivox = fix( ( A(j+1, 3:5, i) + [99.99999,99.99999, 9.99999] ) / 2 ) + 1;
            photon = [i, A(j+1, 2, i), ivox];
        else
            ivox = fix( ( A(1, 3:5, i) + [99.99999,99.99999, 9.99999] ) / 2 ) + 1;
